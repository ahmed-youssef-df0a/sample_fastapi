...
def get_post(db: Session, post_id: int):
    return db.query(models.Posts, func.count(models.Vote.post_id).label("vote_count") ).join(
                models.Vote, models.Vote.post_id == models.Posts.id, isouter=True).group_by(models.Posts.id).filter(
                models.Posts.id == post_id).first()
                

def get_posts(db: Session, skip: int = 0, limit: int = 100, search : Optional[str] = ""):
    posts = db.query(models.Posts, func.count(models.Vote.post_id).label("vote_count") ).join(
        models.Vote, models.Vote.post_id == models.Posts.id, isouter=True).group_by(models.Posts.id).filter(
        models.Posts.title.contains(search)).order_by(models.Posts.created_at.asc()).offset(skip).limit(limit).all()
